...
class RecipeService(
    ...
    fun getAllRecipes(pageable: Pageable): Page<RecipeSummaryDto> = recipeRepository.findAll(pageable).map { it.toRecipeSummaryDto() }

    @Transactional(readOnly = true)
    fun getRecipesByUser(
        userEmail: String,
        pageable: Pageable,
    ): Page<RecipeSummaryDto> {
    ...
    fun getRecipesByCategory(
        ...
        return recipeRepository
            .findByCategoryId(categoryUuid, pageable)
            .map { it.toRecipeSummaryDto() }
    }
    ...
    @Transactional(readOnly = true)
    fun getRecipeDetail(recipeId: UUID): RecipeDetailDto = findRecipeByIdOrThrow(recipeId).toRecipeDetailDto()

    @Transactional(readOnly = true)
    fun getReviewsForRecipe(
        recipeId: UUID,
        pageable: Pageable,
    ): Page<ReviewResponse> {
    ...
    private fun buildRecipeIngredients(
        recipe: Recipe,
        request: RecipeRequest,
            ...
            .map { reqIngredient ->
                val ingredientEntity =
                    ingredientRepository
                        .findById(reqIngredient.ingredientId)
                        .orElseThrow {
    ...
    private fun buildRecipeCategories(
        recipe: Recipe,
